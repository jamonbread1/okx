...
核心逻辑:
  1. RSI 进入超卖/超买区 (RSI<30 / RSI>70)
  2. 价格触碰 BB 下/上轨
  3. ADX < max_adx (确认震荡/弱趋势，非趋势)
  4. EMA 趋势过滤 (以 ATR 为单位的偏离距离，避免逆强趋势)
  5. 出场: RSI 归位 (>60/<40) 由引擎层 PositionManager 处理（见 v2/engine.py）
...
from v2.indicators import bollinger_bands, rsi, atr, adx, ema
...
class MeanReversion(StrategyBase):
    ...
    def __init__(self, cfg: dict):
        super().__init__(cfg)
        self.bb_period = int(cfg.get("mr_bb_period", 20))
        self.rsi_period = int(cfg.get("mr_rsi_period", 14))
        self.rsi_oversold = float(cfg.get("mr_rsi_oversold", 30.0))
        self.rsi_overbought = float(cfg.get("mr_rsi_overbought", 70.0))
        self.max_adx = float(cfg.get("mr_max_adx", 25.0))
        self.ema_filter = int(cfg.get("mr_ema_filter", 50))
        # EMA 偏离阈值（ATR 单位），替代原 ±5% 的过宽容忍带
        self.ema_atr_mult = float(cfg.get("mr_ema_atr_mult", 2.5))
        self.position_pct = float(cfg.get("mr_position_pct", 0.04))
        self.sl_atr_mult = float(cfg.get("mr_sl_atr_mult", 2.0))
        self.tp_atr_mult = float(cfg.get("mr_tp_atr_mult", 1.5))
        # 净期望 R 数阈值（见 StrategyBase._expected_r）
        self.min_expectancy = float(cfg.get("mr_min_expectancy", 0.05))
        self.slippage_pct = float(cfg.get("mr_slippage_pct", 0.0003))
        self.fee_rt = float(cfg.get("mr_fee_rt", 0.0007))
