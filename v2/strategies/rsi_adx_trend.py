...
from v2.indicators import adx, atr, ema, rsi
...
class RsiAdxTrend(StrategyBase):
    ...
    def __init__(self, cfg: dict):
        super().__init__(cfg)
        self.sma_period = int(cfg.get("radx_sma_period", 50))
        self.ema_period = int(cfg.get("radx_ema_period", 7))
        self.rsi_period = int(cfg.get("radx_rsi_period", 2))
        self.adx_period = int(cfg.get("radx_adx_period", 2))
        self.position_pct = float(cfg.get("radx_position_pct", 0.03))
        self.sl_atr_mult = float(cfg.get("radx_sl_atr_mult", 3.0))
        # 用极远 TP，实际退出主要由 v2.engine 中的 RSI<ADX 条件控制。
        self.tp_r = float(cfg.get("radx_tp_r", 999.0))
        self.min_conf = float(cfg.get("radx_min_confidence", cfg.get("min_open_confidence", 0.55)))
        self.fee_rt = float(cfg.get("radx_fee_rt", 0.0010))
        self.slippage_pct = float(cfg.get("radx_slippage_pct", 0.0003))
        self.min_atr_pct = float(cfg.get("radx_min_atr_pct", 0.002))
        self.max_atr_pct = float(cfg.get("radx_max_atr_pct", 0.12))
        self.long_only = bool(cfg.get("radx_long_only", True))
