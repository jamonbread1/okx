...
from v2.indicators import macd, rsi, adx, atr
...
class MacdDivergence(StrategyBase):
    ...
    def __init__(self, cfg: dict):
        super().__init__(cfg)
        self.fast = int(cfg.get("macd_fast", 12))
        self.slow = int(cfg.get("macd_slow", 26))
        self.signal = int(cfg.get("macd_signal", 9))
        self.rsi_low = float(cfg.get("macd_rsi_low", 65.0))
        self.rsi_high = float(cfg.get("macd_rsi_high", 35.0))
        self.min_adx = float(cfg.get("macd_min_adx", 20.0))
        self.position_pct = float(cfg.get("macd_position_pct", 0.04))
        self.sl_atr_mult = float(cfg.get("macd_sl_atr_mult", 1.8))
        self.tp_atr_mult = float(cfg.get("macd_tp_atr_mult", 3.0))
        self.slippage_pct = float(cfg.get("macd_slippage_pct", 0.0003))
