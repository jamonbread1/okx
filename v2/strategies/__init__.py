...
自定义策略
----------
1. 在 ``v2/strategies/`` 新建 ``my_strategy.py``（不要以 ``_`` 开头）
2. 继承 ``StrategyBase``，设置唯一 ``name`` 并实现 ``generate``
3. 在 ``config.yaml`` → ``enabled_strategies`` 加入你的 ``name``
4. ``python -m v2.run_backtest --list-strategies`` 确认已发现
...
from v2.strategies.registry import (
    all_strategies,
    create,
    discover,
    get,
