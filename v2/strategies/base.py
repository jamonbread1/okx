...
class Signal:
    ...
    confidence: 0.0~1.0，策略对信号的置信度
    strategy:  策略名 (vol / mr / rng / ewmac / macd / don)
    ...
    # 引擎/回测层建议的最差成交价（intrabar 止损/止盈保守价）；0 = 由回测用默认价
    fill_price: float = 0.0
    # 分批止盈的 RR 档位（与 batch_ratios 一一对应），用于开仓时冻结档位
...
class StrategyBase:
    ...
    """
    name: str = "base"
    required_regime: str = "any"  # any / trend / chop / mixed
    ...
    def _calc_size(
        ...
        """计算建仓张数（P2-1：kelly 或 voltarget，由配置决定）。"""
        from v2.utils.sizing import calc_size
