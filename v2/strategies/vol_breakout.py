...
from v2.indicators import bollinger_bands, bbw, rsi, atr
...
class VolBreakout(StrategyBase):
    ...
    def __init__(self, cfg: dict):
        super().__init__(cfg)
        self.bb_period = int(cfg.get("vol_bb_period", 20))
        self.bbw_squeeze = float(cfg.get("vol_bbw_squeeze", 0.035))
        self.vol_mult = float(cfg.get("vol_volume_mult", 1.3))
        self.rsi_low = float(cfg.get("vol_rsi_low", 35.0))
        self.rsi_high = float(cfg.get("vol_rsi_high", 65.0))
        self.breakout_buffer = float(cfg.get("vol_breakout_buffer", 0.0003))
        self.position_pct = float(cfg.get("vol_position_pct", 0.05))
        self.sl_atr_mult = float(cfg.get("vol_sl_atr_mult", 1.8))
        self.tp_atr_mult = float(cfg.get("vol_tp_atr_mult", 3.5))
        # 净期望 R 数阈值（见 StrategyBase._expected_r）
        self.min_expectancy = float(cfg.get("vol_min_expectancy", 0.1))
        self.slippage_pct = float(cfg.get("vol_slippage_pct", 0.00035))
        self.fee_rt = float(cfg.get("vol_fee_rt", 0.0007))
        self.short_penalty = float(cfg.get("vol_short_penalty", 0.7))
        # squeeze 需持续根数（当前根之前）
        self.squeeze_hold = int(cfg.get("vol_squeeze_hold", 3))
