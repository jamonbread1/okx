...
def _session() -> requests.Session:
    s = requests.Session()
    s.headers.update({
        "User-Agent": "okx-quant-v3/data-manager",
...
def download_range(
    ...
    """
    bar = normalize_bar(bar)
    # 保持 naive UTC：end/start 在比较时是 naive datetime，now 也得 naive
    # Python 3.12+ 提示 utcnow() deprecated 但行为一致；3.13+ 才移除
...
def download_newer(
    ...
    # 保持 naive UTC（与 since_ts 同型，避免 TypeError）
    # Python 3.12+ 提示 utcnow() deprecated，但行为一致；3.13+ 才移除。
    now = datetime.now(timezone.utc).replace(tzinfo=None)
    if since_ts >= now:
        return pd.DataFrame(columns=["ts", "open", "high", "low", "close", "vol"])
...
def main():
    ap = argparse.ArgumentParser(
        description="OKX K 线统一管理：下载 / 校验 / 入库 / 增量更新（v3）"
    )
