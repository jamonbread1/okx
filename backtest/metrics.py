...
def check_overfitting(
    ...
    max_return_decay: float = 0.55,
    min_oos_trades: int = 15,
) -> Dict:
    """
    Simple practical overfit / distortion checks.
    """
    warnings = []
    is_sh = float(is_metrics.get("sharpe") or 0)
    oos_sh = float(oos_metrics.get("sharpe") or 0)
    is_ret = float(is_metrics.get("total_return") or 0)
    oos_ret = float(oos_metrics.get("total_return") or 0)
    ...
    if is_ret > 0.02:
        decay_r = 1.0 - (oos_ret / max(is_ret, 1e-9))
        if decay_r > max_return_decay:
            warnings.append(f"Return decay {decay_r:.1%} > {max_return_decay:.0%} (IS={is_ret:.2%} OOS={oos_ret:.2%})")
    ...
    if is_ret > 0:
        score += min(1.0, max(0, 1.0 - oos_ret / max(is_ret, 1e-9))) * 0.3
