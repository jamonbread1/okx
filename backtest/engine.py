...
`python -m v3.run_backtest`。本文件仅保留兼容，不再修复新功能。
...
from strategy import StrategyEngine  # v3 引擎
...
class BacktestEngine:
    def __init__(
        ...
        log.warning(
            "BacktestEngine(单品种) 已弃用，请改用 multi_engine / python -m v3.run_backtest"
        )
        self.config = config
        self.store = store
        self.symbol = symbol
        ...
        # v3 引擎：不再传入 fetcher，df 由 run() 组装后直传
    ...
    def _execute(self, sig: TradeSignal, price: float, ts) -> None:
        """v3 Signal 执行。成交后同步回 v3 引擎（confirm_*）。"""
        act = sig.action
        if act == "hold":
            return
        if act == "close":
