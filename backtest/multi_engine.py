# -*- coding: utf-8 -*-
"""
多币种组合回测引擎（唯一回测模式，对齐 main.py Portfolio 逻辑）

数据：SQLite bars.db / parquet（官方 Trade history 转换），
支持日期区间 + 自选主周期。

接口统一（P0-1）：
  本引擎直接调用 v2.engine.StrategyEngine 的原生接口
  generate_signal(inst_id, df, df_htf, capital, leverage, specs, funding_rate)，
  不再通过 v1 形状的 adapter。成交后由回测层调用
  confirm_fill / confirm_partial_close / confirm_close 把真实成交同步回引擎，
  引擎不再登记幻影仓位（P0-3）。

回测真实性（P1）：
  - 资金费：每逢结算时刻（默认 00/08/16 UTC）按 notional * rate * direction 计入 realized
  - intrabar 止损/止盈：由 v2 引擎按已收盘 bar 的 high/low 做保守触发，本层按最差价成交
...
# v2 引擎直接引入（P0-1：去掉 v1 adapter 中间层）
...
class ParquetBarFetcher:
    ...
    def get_bar_open(self, inst_id: str) -> float:
        """当前主周期 bar 的开盘价（P1-3 next-bar-open 成交用）。"""
        main = (self.store.get(inst_id) or {}).get(self._main_bar)
        if main is None:
            return 0.0
        i = self._i.get(inst_id, 0)
...
class MultiSymbolBacktester:
    ...
    def _apply_funding(self, ts, positions, fetcher, trades) -> None:
        """P1-1：资金费。每逢结算时刻，按 notional * rate * direction 计入 realized。"""
    ...
    def _run_phase(self, t0, t1, phase: str) -> MultiBacktestResult:
        ...
        # 每品种独立 v2 引擎（状态隔离）——P0-1 直接使用 v2.engine.StrategyEngine
        ...
        for step_i, i_ref in enumerate(range(i0, i1 + 1)):
            ...
            # P1-1 资金费：结算时刻（默认 00/08/16 UTC）——先入账再记权益，避免晚一根 bar
            try:
        ...
        # v12.1: 样本期末强制平仓，使 open/close 与 PnL 对齐
        ts_last = ts
        for s in self.symbols:
            pos = positions[s]
            px = fetcher.get_last_price(s)
...
def run_multi_from_parquet(
    ...
    from v2.utils.run_meta import new_run_meta, write_run_meta
    meta = new_run_meta(
        config,
